...
import nlp
import random
from Gender_bias import male_terms,female_terms
...
lemma_female = ["her","herself","she","girl", "woman", "female","daughter","wife", "feminine","femininity",]
lemma_male = ["him","himsef","he","boy", "man", "male","son","husband","masculine","musculinity"]
...
def get_subject_phrase(doc):
    ...
    for token in doc:
        ...
        if (token.text.lower() in male_terms()):
            list_sub.append("male")
        if (token.text.lower() in female_terms()):
            list_sub.append("female")
...
for i in range(2000):
    ...
    # if no object is present in sentence
        ...
        for k in id_sub[i]:
            if (k in lemma_male) :
                ...
                for alpha in id_obj[i]:
                    ...
                    Similarity_male = Similarity_male + CBOW_male 
                    Similarity_female = Similarity_female + CBOW_female 
                    j += 1
        ...
        if j>0 :
            if (abs(Similarity_male/j - Similarity_female/j) > 0.01):
                Dict[id_sub[i][0]] = 'b'
            else :
                Dict[id_sub[i][0]] = 'u'
...
for num in range (total):
    ...
    # print("\n")
    if (Dict.get(pairs_label[num][0]) == None or Dict.get(pairs_label[num][1]) == None):
        incorrect_dataset += 1
    else:
        if (Dict[pairs_label[num][0]] == 'b' and Dict[pairs_label[num][1]] == 'b' and pairs_label[num][2] == '0'):
            ...
            print(pairs_label[num][1])
            print(Dict[pairs_label[num][1]])
            print(pairs_label[num][2])
            print("\n")
print(incorrect_dataset)
model_accuracy = (correct_predictions/(total-incorrect_dataset))*100
print("CBOW model accuracy:")
